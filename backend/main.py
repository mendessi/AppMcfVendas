...
async def generate_response(request: Request):
    try:
        # Log da requisição
        logger.info(f"Recebida requisição POST /api/ai/generate")
        logger.info(f"Headers: {dict(request.headers)}")
        
        # Obtém o corpo da requisição
        body = await request.json()
        logger.info(f"Body recebido (raw): {body}")
        logger.info(f"Body recebido (tipo): {type(body)}")
        logger.info(f"Body recebido (keys): {body.keys() if isinstance(body, dict) else 'não é dict'}")
        ...
        try:
            ...
            if not isinstance(body["user_input"], str):
                raise ValueError("O campo 'user_input' deve ser uma string")
            
            if not body["user_input"].strip():
                raise ValueError("O campo 'user_input' não pode estar vazio")
            
            user_input = UserInput(**body)
            logger.info(f"Dados validados: {user_input.dict()}")
        except Exception as e:
            logger.error(f"Erro na validação: {str(e)}")
            logger.error(f"Erro na validação (tipo): {type(e)}")
            logger.error(f"Erro na validação (args): {e.args}")
            return JSONResponse(
        ...
        if not empresa_codigo:
            logger.error("Header x-empresa-codigo não encontrado")
            return JSONResponse(
                status_code=status.HTTP_400_BAD_REQUEST,
                content={"detail": "Código da empresa não fornecido"}
            )

        # Gera resposta
        response = await ai_manager.generate_response(user_input.user_input, request)
